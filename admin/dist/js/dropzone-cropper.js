...
var myDropzone_pdf = new Dropzone("#dropzone_pdf",
        {
            ...
            success: function(file, serverFileName) {
                ...
                j++;
                $('#pdf_file_list').val(JSON.stringify(fileList_pdfs))
            }
        }
);
...
myDropzone_pdf.on('removedfile', function(file) {
    var rmvFile = "";
    for (var f = 0; f < fileList_pdfs.length; f++) {
        if (fileList_pdfs[f].fileName == file.name)
        {
            rmvFile = fileList_pdfs[f].serverFileName;
        }
    }
    if (rmvFile) {
        $.ajax({
            url: "ajax/ajax_file_upload.php?action=delete&file_type=pdf&file_for=product",
            type: "POST",
            data: {
                "fileList_images": rmvFile
            }
